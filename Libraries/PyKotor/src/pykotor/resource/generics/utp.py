...
def construct_utp(
    ...
    root = gff.root
    utp.tag = root.acquire("Tag", "", field_type=GFFFieldType.String)
    utp.name = root.acquire("LocName", LocalizedString.from_invalid(), field_type=GFFFieldType.LocalizedString)
    utp.resref = root.acquire("TemplateResRef", ResRef.from_blank(), field_type=GFFFieldType.ResRef)
    utp.auto_remove_key = bool(root.acquire("AutoRemoveKey", 0, field_type=GFFFieldType.UInt8))
    utp.lock_dc = root.acquire("CloseLockDC", 0, field_type=GFFFieldType.UInt8)
    utp.conversation = root.acquire("Conversation", ResRef.from_blank(), field_type=GFFFieldType.ResRef)
    utp.faction_id = root.acquire("Faction", 0, field_type=GFFFieldType.UInt32)
    utp.plot = bool(root.acquire("Plot", 0, field_type=GFFFieldType.UInt8))
    utp.not_blastable = bool(root.acquire("NotBlastable", 0, field_type=GFFFieldType.UInt8))
    utp.min1_hp = bool(root.acquire("Min1HP", 0, field_type=GFFFieldType.UInt8))
    utp.key_required = bool(root.acquire("KeyRequired", 0, field_type=GFFFieldType.UInt8))
    utp.lockable = bool(root.acquire("Lockable", 0, field_type=GFFFieldType.UInt8))
    utp.locked = bool(root.acquire("Locked", 0, field_type=GFFFieldType.UInt8))
    utp.unlock_dc = root.acquire("OpenLockDC", 0, field_type=GFFFieldType.UInt8)
    utp.key_name = root.acquire("KeyName", "", field_type=GFFFieldType.String)
    utp.animation_state = root.acquire("AnimationState", 0, field_type=GFFFieldType.UInt8)
    utp.appearance_id = root.acquire("Appearance", 0, field_type=GFFFieldType.UInt32)
    utp.maximum_hp = root.acquire("HP", 0, field_type=GFFFieldType.Int16)
    utp.current_hp = root.acquire("CurrentHP", 0, field_type=GFFFieldType.Int16)
    utp.hardness = root.acquire("Hardness", 0, field_type=GFFFieldType.UInt8)
    utp.fortitude = root.acquire("Fort", 0, field_type=GFFFieldType.UInt8)
    utp.on_closed = root.acquire("OnClosed", ResRef.from_blank(), field_type=GFFFieldType.ResRef)
    utp.on_damaged = root.acquire("OnDamaged", ResRef.from_blank(), field_type=GFFFieldType.ResRef)
    utp.on_death = root.acquire("OnDeath", ResRef.from_blank(), field_type=GFFFieldType.ResRef)
    utp.on_heartbeat = root.acquire("OnHeartbeat", ResRef.from_blank(), field_type=GFFFieldType.ResRef)
    utp.on_lock = root.acquire("OnLock", ResRef.from_blank(), field_type=GFFFieldType.ResRef)
    utp.on_melee_attack = root.acquire("OnMeleeAttacked", ResRef.from_blank(), field_type=GFFFieldType.ResRef)
    utp.on_open = root.acquire("OnOpen", ResRef.from_blank(), field_type=GFFFieldType.ResRef)
    utp.on_force_power = root.acquire("OnSpellCastAt", ResRef.from_blank(), field_type=GFFFieldType.ResRef)
    utp.on_unlock = root.acquire("OnUnlock", ResRef.from_blank(), field_type=GFFFieldType.ResRef)
    utp.on_user_defined = root.acquire("OnUserDefined", ResRef.from_blank(), field_type=GFFFieldType.ResRef)
    utp.has_inventory = bool(root.acquire("HasInventory", 0, field_type=GFFFieldType.UInt8))
    utp.party_interact = bool(root.acquire("PartyInteract", 0, field_type=GFFFieldType.UInt8))
    utp.static = bool(root.acquire("Static", 0, field_type=GFFFieldType.UInt8))
    utp.useable = bool(root.acquire("Useable", 0, field_type=GFFFieldType.UInt8))
    utp.on_end_dialog = root.acquire("OnEndDialogue", ResRef.from_blank(), field_type=GFFFieldType.ResRef)
    utp.on_inventory = root.acquire("OnInvDisturbed", ResRef.from_blank(), field_type=GFFFieldType.ResRef)
    utp.on_used = root.acquire("OnUsed", ResRef.from_blank(), field_type=GFFFieldType.ResRef)
    utp.on_open_failed = root.acquire("OnFailToOpen", ResRef.from_blank(), field_type=GFFFieldType.ResRef)
    utp.comment = root.acquire("Comment", "", field_type=GFFFieldType.String)
    utp.unlock_diff = root.acquire("OpenLockDiff", 0, field_type=GFFFieldType.UInt8)
    utp.unlock_diff_mod = root.acquire("OpenLockDiffMod", 0, field_type=GFFFieldType.Int8)
    ...
    for item_struct in item_list:
        ...
        utp.inventory.append(item)
        item.pos_x = item_struct.acquire("Repos_PosX", 0)
        item.pos_y = item_struct.acquire("Repos_Posy", 0)

    utp.description = root.acquire("Description", LocalizedString.from_invalid(), field_type=GFFFieldType.LocalizedString)
    utp.interruptable = bool(root.acquire("Interruptable", 0, field_type=GFFFieldType.UInt8))
    utp.portrait_id = root.acquire("PortraitId", 0, field_type=GFFFieldType.UInt16)
    utp.trap_detectable = bool(root.acquire("TrapDetectable", 0, field_type=GFFFieldType.UInt8))
    utp.trap_detect_dc = root.acquire("TrapDetectDC", 0, field_type=GFFFieldType.UInt8)
    utp.trap_disarmable = bool(root.acquire("TrapDisarmable", 0, field_type=GFFFieldType.UInt8))
    utp.trap_disarm_dc = root.acquire("DisarmDC", 0, field_type=GFFFieldType.UInt8)
    utp.trap_flag = root.acquire("TrapFlag", 0, field_type=GFFFieldType.UInt8)
    utp.trap_one_shot = bool(root.acquire("TrapOneShot", 0, field_type=GFFFieldType.UInt8))
    utp.trap_type = root.acquire("TrapType", 0, field_type=GFFFieldType.UInt8)
    utp.will = root.acquire("Will", 0, field_type=GFFFieldType.UInt8)
    utp.reflex = root.acquire("Ref", 0, field_type=GFFFieldType.UInt8)
    utp.on_disarm = root.acquire("OnDisarm", ResRef.from_blank(), field_type=GFFFieldType.ResRef)
    utp.on_trap_triggered = root.acquire("OnTrapTriggered", ResRef.from_blank(), field_type=GFFFieldType.ResRef)
    utp.bodybag_id = root.acquire("BodyBag", 0, field_type=GFFFieldType.UInt8)
    utp.type_id = root.acquire("Type", 0, field_type=GFFFieldType.UInt8)
    utp.palette_id = root.acquire("PaletteID", 0, field_type=GFFFieldType.UInt8)

    utp.portrait = root.acquire("Portrait", ResRef.from_blank(), field_type=GFFFieldType.ResRef)

    remember_gff(utp, gff)
    return utp
...
def dismantle_utp(
    ...
    *,
    ...
    root.set_resref("Portrait", utp.portrait)

    return preserve_gff(utp, gff, lambda original: dismantle_utp(original, game=game, use_deprecated=use_deprecated))
